...
import org.smartapartment.smartapartment.mapper.CheckInMapper;
...
public class RoomService {
     ...
     * @return 房间列表
     */
    public List<Room> getAvailableRooms(Long buildingId, Integer roomType, Integer floorNumber, Boolean hasAvailableBeds) {
        long startTime = System.currentTimeMillis();
        String dataSource = "未知";
        
        // 尝试从特定筛选条件缓存获取
        String specificCacheKey = buildCacheKey(ROOM_AVAILABLE_CACHE_KEY, buildingId, roomType, floorNumber, hasAvailableBeds);
        ...
        if (hasFilterConditions(buildingId, roomType, floorNumber, hasAvailableBeds)) {
            ...
            if (allRooms != null) {
                ...
                List<Room> filteredRooms = filterRoomsInMemory(allRooms, buildingId, roomType, floorNumber, hasAvailableBeds);
                ...
                if (!filteredRooms.isEmpty()) {
                    // 将筛选结果存入特定条件缓存
                    redisTemplate.opsForValue().set(specificCacheKey, filteredRooms, CACHE_EXPIRE_TIME, TimeUnit.MINUTES);
     ...
     * 判断是否有任何筛选条件
     */
    private boolean hasFilterConditions(Long buildingId, Integer roomType, Integer floorNumber, Boolean hasAvailableBeds) {
     ...
     * 在内存中筛选房间列表
     */
    private List<Room> filterRoomsInMemory(List<Room> allRooms, Long buildingId, Integer roomType, Integer floorNumber, Boolean hasAvailableBeds) {
        return allRooms.stream()
            .filter(room -> buildingId == null || (room.getBuildingId() != null && room.getBuildingId().equals(buildingId)))
